...
Source_Folder_Path = "/opt/mnt/publicdrive/Analytics/Gerard/MarketingProjects/ItemVideoPriorities/"
os.chdir(Source_Folder_Path)

#UGItemData = pd.read_csv("RevenueAndQuantityDataMissedGAQueries.csv")
...
#Results_File = openpyxl.load_workbook('/opt/mnt/publicdrive/Analytics/Gerard/MarketingProjects/ItemVideoPriorities/Video_Priorities_Results_Missed_GA_Queries.xlsx')
Results_File = openpyxl.load_workbook('/opt/mnt/publicdrive/Analytics/Gerard/MarketingProjects/ItemVideoPriorities/Video_Priorities_Results.xlsx')
Results_Tab = Results_File.get_sheet_by_name('Results')

#trying to work around w drive issues saving issues
...
#Results_File.save('Video_Priorities_Results_Missed_GA_Queries.xlsx')
...
while x <= DataFrame_Number_Rows:
        ...
        Item_name_raw = query_parameters.iloc[4]
        ...
        #Calc Avg Page Views Per Month (column 7)
        try:
            Average_Views_Per_Month = int(int(All_Time_Page_Views)/int(Total_Months))
        except ZeroDivisionError:
            Average_Views_Per_Month = 0

        #Calc Average Revenue Per Month (Column 8)

        try:
            Average_Revenue_Per_Month = int(int(Total_Sales) / int(Total_Months))
        except ZeroDivisionError:
            Average_Revenue_Per_Month = 0

        #Calc Average Unit Sales per month (column 9)
        try:
            Average_Unit_Sales_Per_Month = int(int(Total_Quantity_Sold) / int(Total_Months))
        except ZeroDivisionError:
            Average_Unit_Sales_Per_Month = 0


        #Results_File = openpyxl.load_workbook('Video_Priorities_Results_Missed_GA_Queries.xlsx')
        Results_File = openpyxl.load_workbook('Video_Priorities_Results.xlsx')
        ...
        # column 7 name Average Views Per Month
        col = col + 1
        Results_Tab.cell(row=empty_row, column=col).value = Average_Views_Per_Month
        Results_Tab.cell(row=empty_row, column=col).number_format = "#######"

        # column 8 name Average Revenue Per Month
        col = col + 1
        Results_Tab.cell(row=empty_row, column=col).value = Average_Revenue_Per_Month
        Results_Tab.cell(row=empty_row, column=col).number_format = "#######"

        # column 9 name Average Unit Sales per month
        col = col + 1
        Results_Tab.cell(row=empty_row, column=col).value =  Average_Unit_Sales_Per_Month
        Results_Tab.cell(row=empty_row, column=col).number_format = "#######"
        ...
        #
        try:
            Geometric_mean = geometric_mean([Average_Revenue_Per_Month,Average_Views_Per_Month])
        except ZeroDivisionError:
            Geometric_mean = 0
        # ----------------------------------------------------------------------------------------------------------------------
